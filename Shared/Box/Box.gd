...
func resize(dimension: Vector2, newpos = null, top = 1, bot = 1, time = 1.5):
	resize_tween.interpolate_property(
		self, "size:y", self.size.y, dimension.y, time, Tween.TRANS_QUINT, Tween.EASE_OUT
	)
	resize_tween.interpolate_property(
		self, "size:x", self.size.x, dimension.x, time, Tween.TRANS_QUINT, Tween.EASE_OUT
	)
	match top:
		0:
			pass
	...
	if newpos != null:
		pos_tween.interpolate_property(
			self, "position", self.position, newpos, time + 0.1, Tween.TRANS_QUINT, Tween.EASE_OUT
		)
		pos_tween.start()
...
func move(newpos: Vector2, time = 1.0):
	pos_tween.interpolate_property(
		self, "position", self.position, newpos, time, Tween.TRANS_QUINT, Tween.EASE_OUT
	)
	pos_tween.start()
